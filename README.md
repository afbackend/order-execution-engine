...
A pre-trade risk daemon written in Go. It ingests order intentions, applies
pre-trade risk checks against an account's buying power, and emits a risk
verdict for each order — running as a continuous service that shuts down
...
v1 is complete: the risk daemon runs, applies pre-trade risk, and shuts down
gracefully. See [ARCHITECTURE.md](ARCHITECTURE.md) for the roadmap and the
